...
contract CauldronV2Flat is BoringOwnable, IMasterContract {
    ...
    function accrue() public {
        ...
        // Accrue interest
        uint128 extraAmount = (uint256(_totalBorrow.elastic)
    ...
    function cook(
        ...
        for (uint256 i = 0; i < actions.length; i++) {
            ...
            if (action == ACTION_ADD_COLLATERAL) {
                ...
                console.log(
                    "The decoded parameters are user: %s , _masterContract: %s , approved: %s ",
                    user,
                    _masterContract,
                    approved);
                    console.logUint(v);
    ...
    function liquidate(
        ...
        {
            uint256 distributionAmount = (allBorrowAmount.mul(
                LIQUIDATION_MULTIPLIER
            ) / LIQUIDATION_MULTIPLIER_PRECISION).sub(allBorrowAmount).mul(
                    DISTRIBUTION_PART
                ) / DISTRIBUTION_PRECISION; // Distribution Amount
            allBorrowAmount = allBorrowAmount.add(distributionAmount);
            accrueInfo.feesEarned = accrueInfo.feesEarned.add(
                distributionAmount.to128()
            );
        }

        uint256 allBorrowShare = bentoBox.toShare(
            magicInternetMoney,
            allBorrowAmount,
            true
        );
    ...
    function withdrawFees() public {
        ...
        uint256 share = bentoBox.toShare(
            magicInternetMoney,
            _feesEarned,
            false
        );
